    ...
    public class SkipNight : RustPlugin
    {
        ...
        private void StartVote()
        {
            ...
            else
            {
                ...
                {
                    var player = BasePlayer.activePlayerList[i];
                    Reply(player, GetMsg("StartVoteChat", player.UserIDString).Replace("{0}", _config.Command));
                }
            }
        ...
        private void CommandChatVote(BasePlayer player, string command, string[] args)
        {
            ...
            if (args.Length < 1)
            {
                Reply(player, GetMsg("NoArguments", userId).Replace("{0}", _config.Command));
                return;
            }
            ...
            switch (args[0])
            {
                ...
                case "help":
                {
                    Reply(player, GetMsg("Help", userId).Replace("{0}", _config.Command), header:"Помощник по командам");
                    return;
                }
