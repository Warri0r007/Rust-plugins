...
{
    [Info("NameRewards", "Vlad-00003", "1.2.2", ResourceId = 197)]
    ...
    class NameRewards : CovalencePlugin
    {
        ...
        private class PluginConfig
        {
            ...
            [JsonProperty("Аватар сообщений в чате")]
            public ulong? ChatAvatar;
        ...
        private bool CheckConfig()
        {
            ...
            //==version < 1.2.0==
            if (_config.ChatAvatar == null)
            {
                PrintWarning("Config file updated: chat avatar option added");
                _config.ChatAvatar = 0ul;
                res = true;
            }
            return res;
        }
        ...
        private void SendResponse(IPlayer player, string langKey, params object[] args)
        {
            ...
            format = string.Format(_config.ChatFormat, format);
#if RUST
            var bPlayer = player.Object as BasePlayer;
            if (bPlayer)
                bPlayer.SendConsoleCommand("chat.add", 2, _config.ChatAvatar ?? 0ul, Formatter.ToUnity(format));
            else
                player.Reply(format);
        ...
        private string GetMsg(string key, ulong playerid = 0) =>
