...
{
    [Info ("Trade", "Scalbox, Calytic", "1.2.46")] // Hotfix on Line 999 for May 2023 Rust Update - Solarix
    class Trade : RustPlugin
    {
        ...
        [PluginReference]
        private Plugin Ignore, Clans, Friends;

        private Hash<BasePlayer, OnlinePlayer> onlinePlayers = new();
        private Dictionary<string, DateTime> tradeCooldowns = new();
        ...
        private void PopulateOnlinePlayers()
        {
            foreach (var player in BasePlayer.activePlayerList.Where(player => !onlinePlayers.ContainsKey(player)))
            {
                onlinePlayers.Add(player, new OnlinePlayer(player));
            }
        }
        ...
        {
            if (!onlinePlayers.TryGetValue(player, out _))
            {
                onlinePlayers.Add(player, new OnlinePlayer(player));
            }
...
";
        ...
        void OpenBoxView (BasePlayer player, BaseEntity targArg)
        {
            ...
            if (targArg is BasePlayer) {
                ...
                if (container.uid.Value == 0) /* Changed (container.uid == 0) to (container.uid.Value == 0) - Solarix */
                    container.GiveUID ();

                view.enableSaving = false;
                view.Spawn ();
                view._inventory = container;
