...
namespace Oxide.Plugins
{
	...
	public class Stacks : RustPlugin
	{
		...
		private const string 
			CMD_Main_Console = "UI_Stacks",
            Layer = "UI.Stacks",
            AdminPerm = "stacks.admin";

		private readonly Dictionary<ulong, float> _multiplierByItem = new();
		...
		private readonly Dictionary<string, int> _defaultItemStackSize = new();
		...
		private class Configuration
		{
			...
			[JsonProperty(PropertyName = LangRu ? "Изменение множителей в контейнерах при помощи киянки" : "Changing multiplies in containers using a hammer")]
			public bool UserHammer = false;

			[JsonProperty(PropertyName = LangRu ? "Стандартный множитель для новых контейнеров" : "Default Multiplier for new containers")]
			public float DefaultContainerMultiplier = 1f;

			[JsonProperty(PropertyName = LangRu ? "Запретить overstack предметов?" : "Disallow overstack items?")] public bool DisableOverstack = true;
			
			[JsonProperty(PropertyName = LangRu ?  "Заблокированный список" : "Blocked List")]
		...
		{
			[JsonProperty(PropertyName = LangRu ? "Предметы" : "Items", ObjectCreationHandling = ObjectCreationHandling.Replace)]
			public List<string> Items;

			[JsonProperty(PropertyName = LangRu ? "Скины" : "Skins", ObjectCreationHandling = ObjectCreationHandling.Replace)]
		...
		private void LoadData()
		{
			...
			_containers ??= new Dictionary<string, ContainerData>();
				...
				string name = null)
			{
				_image ??= new CuiRawImageComponent
				{
					Png = _instance.ImageLibrary?.Call<string>("GetImage",
						string.IsNullOrEmpty(Image) ? "NONE" : Image) ?? string.Empty
				};
		...
		private class ItemInfo
		{
			...
			[JsonIgnore] public int DefaultStackSize => _instance._defaultStackByID.GetValueOrDefault(itemId, 0);
			...
			{
				return _instance._itemByID.GetValueOrDefault(id);
			}
		...
		private readonly Dictionary<string, string> _constContainers = new()
		{
			...
			["assets/prefabs/deployable/tool cupboard/cupboard.tool.deployed.prefab"] = "https://i.imgur.com/mD9KsAL.png",
			...
			["assets/prefabs/deployable/tool cupboard/retro/cupboard.tool.retro.deployed.prefab"] = "https://i.ibb.co/6RW24LK/image.png",
		};
		...
		private object CanMoveItem(Item movedItem, PlayerInventory playerInventory, ItemContainerId targetContainer,
			int targetSlot, int amount)
		{
			...
			if (targetSlot == -1)
			{
				...
				if (container == null)
				{
					if (_config.DisableOverstack && movedItem.amount > movedItem.info.stackable)
					{
						var loops = 1;
			...
			if (_config.DisableOverstack && container != null)
			{
#if TESTING
		...
		{
#if TESTING
			using (var watch = new StopwatchWrapper("OnMaxStackable took {0}ms."))
#endif
			{
				if (item == null ||
...
#if TESTING
				watch.AdditionalInfo = $"dictionary_count: {_multiplierByItem.Count}";
#endif

				if (_multiplierByItem.TryGetValue(item.uid.Value, out var multiplier))
					return Mathf.FloorToInt(item.info.stackable * multiplier);

				if (item.parent == null || item.parent.entityOwner == null) return null;

				var container = GetContainerMultiplier(item.parent.entityOwner.name);

				return Mathf.FloorToInt(container * item.info.stackable);
			}
		}
		...
		private void OnHammerHit(BasePlayer player, HitInfo info)
		{
			if (!_config.UserHammer || player == null ||
			    !player.HasPermission(AdminPerm) || info == null)
				return;
			...
			if (_containers.TryGetValue(container.name, out var data))
		...
		#region FurnaceSplitter && Image Library

		private void OnPluginLoaded(Plugin plugin)
		{
			switch (plugin.Name)
			{
				case "FurnaceSplitter":
					_hasFurnaceSplitter = true;
					break;
				case "ImageLibrary":
		...
		private void CmdOpenStacks(IPlayer cov, string command, string[] args)
		{
			...
			switch (args[0])
			{
				case "sethandstack":
				{
					if (args.Length < 2 || !int.TryParse(args[1], out var stackSize))
					{
				...
				case "setstack":
				{
					if (args.Length < 3 || !int.TryParse(args[2], out var stackSize))
					{
		...
		[ConsoleCommand("stacks.manage")]
		private void CmdConsoleStacksManage(ConsoleSystem.Arg arg)
		{
			...
			if (!arg.HasArgs())
			{
				...
				sb.AppendLine("Stacks management commands:");
				sb.AppendLine($"– stacks.manage stack set [shortname] [size] – set stack size");
				sb.AppendLine($"– stacks.manage stack get [shortname] – get stack size");
				sb.AppendLine($"– stacks.manage stack get all – get all stack sizes");
			...
			switch (arg.GetString(0))
			{
				case "stack":
				{
					switch (arg.GetString(1))
					{
						case "set":
						{
							var shortName = arg.GetString(2);
							if (string.IsNullOrEmpty(shortName))
							{
								SendReply(arg, "Invalid shortname!");
								return;
							}

							var item = _items.Find(x => x.ShortName == shortName);
							if (item == null)
							{
								SendReply(arg, $"Item '{shortName}' not found!");
								return;
							}
							
							var stackSize  = arg.GetInt(3, -1);
							if (stackSize <= 0)
							{
                                SendReply(arg,  $"Invalid stack size!");
								return;
							}
							...
							SendReply(arg, $"You have changed stack size to {item.CustomStackSize}!");
							break;
						}
						...
						case "get":
						{
							...
							{
								var sb = new StringBuilder();
								sb.AppendLine("All stacks:");
								
								_items.ForEach(item => sb.AppendLine($"{item.ShortName}: {item.CustomStackSize}"));
							...
							else
							{
								...
								SendReply(arg, $"Stack size: {item.CustomStackSize} (0 – default)");
							}
		...
		private void CmdConsoleStacks(ConsoleSystem.Arg arg)
		{
			...
			switch (arg.Args[0])
			{
				case "page":
				{
					int category = -1, page = 0;
					if (!arg.HasArgs(2) || !int.TryParse(arg.Args[1], out var type)) return;
				...
				{
					if (!arg.HasArgs(5) ||
					    !int.TryParse(arg.Args[1], out var type) ||
					    !int.TryParse(arg.Args[2], out var category) ||
					    !int.TryParse(arg.Args[4], out var page)) return;
				...
				case "settings":
				{
					if (!arg.HasArgs(5) ||
					    !int.TryParse(arg.Args[1], out var type) ||
					    !int.TryParse(arg.Args[2], out var category) ||
					    !int.TryParse(arg.Args[3], out var page) ||
				...
				case "apply_settings":
				{
					if (!arg.HasArgs(6) ||
					    ...
					    !int.TryParse(arg.Args[2], out var category) ||
					    !int.TryParse(arg.Args[3], out var page) ||
					    !int.TryParse(arg.Args[4], out var id) ||
					    !float.TryParse(arg.Args[5], out var nowValue) || nowValue <= 0) return;
			...
			bool first = false)
		{
			...
			container.Add(new CuiButton
			{
				...
				Button =
				{
					...
					Command = $"{CMD_Main_Console} page 1"
				}
			}, Layer + ".Header");
			...
			container.Add(new CuiButton
			{
				...
				Button =
				{
					...
					Command = $"{CMD_Main_Console} page 0"
				}
			}, Layer + ".Header");
			...
			container.Add(new CuiElement
			{
				...
				Components =
				{
					new CuiInputFieldComponent
					{
						...
						Align = TextAnchor.MiddleLeft,
						Font = "robotocondensed-regular.ttf",
						Command = $"{CMD_Main_Console} page {type} {category} {page}",
						Color = "1 1 1 0.95",
						CharsLimit = 32,
						NeedsKeyboard = true,
						HudMenuInput = true
				...
				case 0: //Item
				{
					...
					container.Add(new CuiButton
					{
						...
						Button =
						{
							...
							Command = $"{CMD_Main_Console} page {type} {-1}"
						}
					}, Layer + ".Main");
					...
					for (var i = 0; i < _categories.Count; i++)
					{
						...
						container.Add(new CuiButton
						{
							...
							Button =
							{
								...
								Command = $"{CMD_Main_Console} page {type} {i}"
							}
						}, Layer + ".Main");
						...
						container.Add(new CuiButton
							{
								...
								Button =
								{
									Color = HexToCuiColor("#4B68FF"),
									Command =
										$"{CMD_Main_Console} settings {type} {category} {(string.IsNullOrEmpty(search) ? page : 0)} {item.ID}"
								}
							}, Layer + $".Panel.{item.ShortName}");
						...
						container.Add(new CuiButton
							{
								...
								Button =
								{
									Color = HexToCuiColor("#4B68FF"),
									Command =
										$"{CMD_Main_Console} settings {type} {category} {(string.IsNullOrEmpty(search) ? page : 0)} {check.Value.ID}"
								}
							}, Layer + $".Panel.{ySwitch}");
			...
			container.Add(new CuiButton
			{
				...
				Button =
				{
					...
					Command = page != 0 ? $"{CMD_Main_Console} page {type} {category} {page - 1} {search}" : ""
				}
			}, Layer + ".Pages");
			...
			container.Add(new CuiButton
			{
				...
				Button =
				{
					...
					Command = maxCount > (page + 1) * lines
						? $"{CMD_Main_Console} page {type} {category} {page + 1} {search}"
						: ""
				}
			}, Layer + ".Pages");
					...
					Components =
					{
						new CuiInputFieldComponent
						{
							...
							Command = string.IsNullOrEmpty(search) || search.Length < 2
								? $"{CMD_Main_Console} page {type} {category}"
								: $"{CMD_Main_Console} enter_page {type} {category} {search}",
							Color = "1 1 1 0.95",
							CharsLimit = 32,
							NeedsKeyboard = true,
							HudMenuInput = true
			...
			bool first = false)
		{
			...
			container.Add(new CuiElement
			{
				...
				Components =
				{
					new CuiInputFieldComponent
					{
						Text = enterValue > 0 ? $"{enterValue}" : string.Empty,
						Align = TextAnchor.MiddleLeft,
						Font = "robotocondensed-regular.ttf",
						FontSize = 12,
						Color = "1 1 1 0.95",
						CharsLimit = 32,
						Command = $"{CMD_Main_Console} settings {type} {category} {page} {id}",
						NeedsKeyboard = true,
						HudMenuInput = true
					},
					new CuiRectTransformComponent
			...
			container.Add(new CuiButton
			{
				...
				Button =
				{
					...
					Command = enterValue > 0
						? $"{CMD_Main_Console} apply_settings {type} {category} {page} {id} {enterValue}"
						: $"{CMD_Main_Console} page {type} {category} {page}"
				}
			}, Layer + ".Main");
		...
		{
			return _defaultStackByID.TryGetValue(definition.itemid, out var stack) ? stack : definition.stackable;
		}

		private void LoadReferences()
		{
			if (FurnaceSplitter is {IsLoaded: true}) _hasFurnaceSplitter = true;
		}

		private Dictionary<int, string> _containerPrefabByID = new();

		private Dictionary<string, int> _containerPrefabID = new();

		private Dictionary<int, ContainerData> _containerBase = new();

		private void UniqueLoadContainers()
		{
			var loaded = new HashSet<string>();

			foreach (var (prefab, data) in _containers)
			{
				loaded.Add(prefab);
			...
			{
				if (!loaded.Add(container.Key)) continue;
			...
			{
				var prefab = entityPath.ToLower();
				if (string.IsNullOrEmpty(prefab) || loaded.Contains(prefab) || prefab.Contains("radtown") || prefab.Contains("autospawn") || prefab.Contains("roadsigns")) continue;

				var loot = GameManager.server.FindPrefab(prefab)?.GetComponent<StorageContainer>();
				if (loot == null || loot is LootContainer) continue;
				
				var splitedPrefabPath = prefab.Split('/');
				var formatedPrefab = splitedPrefabPath.Length > 0 ? splitedPrefabPath[^1].Replace(".prefab", "").Replace(".entity", "").Replace(".weapon", "").Replace(".deployed", "").Replace("_", ".") : string.Empty;
				
				var data = new ContainerData
				{
					Image = HasImage(formatedPrefab) ? formatedPrefab : string.Empty,
					Multiplier = _config.DefaultContainerMultiplier,
					Prefab = prefab
				};
		...
		{
			return _containerPrefabByID.TryGetValue(id, out var prefab) ? prefab : string.Empty;
		}

		private int GetPrefabID(string prefab)
		{
			return _containerPrefabID.GetValueOrDefault(prefab, 0);
		}
		...
		{
			return _containerBase.GetValueOrDefault(id);
		}
		...
		private void SetContainerMultiplier(int id, float multiplier)
		{
			...
			if (_containers.TryGetValue(prefab, out var data))
			{
		...
		{
#if TESTING
			using (new StopwatchWrapper("GetContainerMultiplier for prefab '" + prefab + "' took {0}ms."))
#endif
			{
		...
		private void LoadCategories()
		{
			...
			{
				if (dict.TryGetValue(info.category, out var category))
					category.Add(info);
				else
		...
		private Dictionary<int, ItemInfo> _itemByID = new();

		private Dictionary<int, int> _defaultStackByID = new();
		...
		protected override void LoadDefaultMessages()
		{
			...
			lang.RegisterMessages(new Dictionary<string, string>
			{
				...
				[CloseButton] = "✕",
				[TitleMenu] = "Stacks",
				[ContainerTitle] = "Контейнер",
				[ItemTitle] = "Предмет",
				[BtnBack] = "◀",
				[BtnNext] = "▶",
				[SearchTitle] = "Поиск...",
				[DefaultStack] = "Стандартный стак",
				[CustomStack] = "Текущий стак",
				[StackFormat] = "x{0}",
				[SettingsTitle] = "Настроить",
				[DefaultMultiplier] = "Стандартный Множитель: <color=white>x{0}</color>",
				[CustomMultiplier] = "Текущий Множитель: <color=white>x{0}</color>",
				[CurrentStack] = "Текущий стак:",
				[CurrentMultiplier] = "Текущий множитель:",
				[EnterStack] = "Введите размер стака:",
				[EnterMultiplier] = "Введите множитель:",
				[AcceptTitle] = "ПРИНЯТЬ",
				[SetStack] = "Вы установили размер стака {0} для '{1}'",
			...
			lang.RegisterMessages(new Dictionary<string, string>
			{
				...
				[StackFormat] = "x{0}",
				[SettingsTitle] = "设置",
				[DefaultMultiplier] = "默认倍增器: <color=white>x{0}</color>",
				[CustomMultiplier] = "当前倍增器: <color=white>x{0}</color>",
				[CurrentStack] = "当前堆叠大小:",
				[CurrentMultiplier] = "当前倍增器:",
				[EnterStack] = "输入新的堆叠大小:",
				[EnterMultiplier] = "输入倍增器:",
				[AcceptTitle] = "接受",
				[SetStack] = "您已将'{1}'的堆叠大小设置为{0}",
				[NoILError] = "插件未正常工作，请联系管理员！",
			}, this, "zh-CN");
			
			lang.RegisterMessages(new Dictionary<string, string>
			{
				[NoPermission] = "Sie haben nicht die erforderliche Berechtigung",
				[AllTitle] = "Alle",
				[CloseButton] = "✕",
				[TitleMenu] = "Stacks",
				[ContainerTitle] = "Behälter",
				[ItemTitle] = "Gegenstand",
				[BtnBack] = "◀",
				[BtnNext] = "▶",
				[SearchTitle] = "Suchen...",
				[DefaultStack] = "Standard Stapelgröße",
				[CustomStack] = "Benutzerdefinierte Stapelgröße",
				[StackFormat] = "x{0}",
				[SettingsTitle] = "Einstellungen",
				[DefaultMultiplier] = "Standard Multiplikator: <color=white>x{0}</color>",
				[CustomMultiplier] = "Aktueller Multiplikator: <color=white>x{0}</color>",
				[CurrentStack] = "Aktuelle Stapelgröße:",
				[CurrentMultiplier] = "Aktueller Multiplikator:",
				[EnterStack] = "Geben Sie die neue Stapelgröße ein:",
				[EnterMultiplier] = "Geben Sie den Multiplikator ein:",
				[AcceptTitle] = "AKZEPTIEREN",
				[SetStack] = "Sie haben die Stapelgröße für '{1}' auf {0} gesetzt",
				[NoILError] = "Das Plugin funktioniert nicht korrekt, kontaktieren Sie den Administrator!",
			}, this, "de");
			
			lang.RegisterMessages(new Dictionary<string, string>
			{
				[NoPermission] = "Vous n'avez pas l'autorisation requise",
				[AllTitle] = "Tout",
				[CloseButton] = "✕",
				[TitleMenu] = "Empilements",
				[ContainerTitle] = "Conteneur",
				[ItemTitle] = "Objet",
				[BtnBack] = "◀",
				[BtnNext] = "▶",
				[SearchTitle] = "Rechercher...",
				[DefaultStack] = "Taille de pile par défaut",
				[CustomStack] = "Taille de pile personnalisée",
				[StackFormat] = "x{0}",
				[SettingsTitle] = "Paramètres",
				[DefaultMultiplier] = "Multiplicateur par défaut: <color=white>x{0}</color>",
				[CustomMultiplier] = "Multiplicateur actuel: <color=white>x{0}</color>",
				[CurrentStack] = "Taille de pile actuelle:",
				[CurrentMultiplier] = "Multiplicateur actuel:",
				[EnterStack] = "Entrez la nouvelle taille de pile:",
				[EnterMultiplier] = "Entrez le multiplicateur:",
				[AcceptTitle] = "ACCEPTER",
				[SetStack] = "Vous avez défini la taille de pile à {0} pour '{1}'",
				[NoILError] = "Le plugin ne fonctionne pas correctement, veuillez contacter l'administrateur !",
			}, this, "fr");
		}
		...
		{
			LogToFile("debug", string.Format(format, time), this);
...
namespace Oxide.Plugins.StacksExtensionMethods
{
	...
	public static class ExtensionMethods
	{
		...
		private static bool IsKilled(this BaseNetworkable a)
		{
		...
		private sealed class ReverseLambdaComparer<T, U> : IComparer<T>
		{
