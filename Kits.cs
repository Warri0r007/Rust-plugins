...
namespace Oxide.Plugins
{
    ...
    class Kits : RustPlugin
    {
        ...
        void CmdConsoleLoadKits(ConsoleSystem.Arg args)
        {
            ...
            if (args.Connection != null)
                args.Player().ConsoleMessage("Киты загружены с папки data/Kits/KitsList.json");
            else
                Puts("Киты загружены с папки data/Kits/KitsList.json");
        }
        [ConsoleCommand("kit")]
        ...
        private void RefreshCooldownKitsUI()
        {
            ...
            foreach (var playerGUIData in OpenGUI)
            {
                ...
                {
                    var playerKitData = playerKitsData[kitname.Name.ToLower()];
        ...
        private void InitilizeCooldown(ref CuiElementContainer container, BasePlayer player, Kit kit, int page)
        {
            ...
            var playerData = GetPlayerData(kit.Name.ToLower(), player.userID);
            container.Add(new CuiPanel
            {
                Image = {
        ...
        private KitData GetPlayerData(string name, ulong playerid = 0)
        {
